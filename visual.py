...
class myGraph():
    ...
    def side_pos(self,G,root=None,height=1,center=(0,0),pos=None,xgap=1):
        if root==None: #first iteration. find all origins
            ...
            for num,i in enumerate(roots):
                pos=self.side_pos(G,root=i,height=1/(len(roots)+1),center=(0,(num+1)/(len(roots)+1)),pos=pos)
            return pos
        if pos==None:
            pos={root:center}
        else:
    ...
    def date_pos(self,G,root=None,height=1,center=(0,0),pos=None,xgap=0.2,orig_date=None):
        if root==None: #first iteration. find all origins
            ...
            dates=[datetime.datetime.strptime(G.nodes(data=True)[i]['birthDate'],'%d-%m-%Y') for i in roots] #a list of the birthdates for each  of the root samples
            orig_date=min(dates) #The earliest of the dates
            for num,i in enumerate(roots):
                pos=self.date_pos(G,root=i,height=1/(len(roots)+1),center=(0,(num+1)/(len(roots)+1)),pos=pos,orig_date=orig_date)
            return pos
        if pos==None:
            pos={root:center}
        else:
    ...
    def __init__(self):
        ...
        self.slider.callback=slidercallback
    ...
    def getFromDB(self,sqlsession):
        ...
        self.renderer.node_renderer.data_source.add([self.colors[Sample.Type[j].value-1] for j in [i[1]['type'] for i in self.nodelist]],'color')
...
class NewSamplePanel():
    def __init__(self):
       ...
       self.dateText=TextInput(title="Creation Date (format 'Day-Month-Year' UTC)",value=datetime.datetime.strftime(datetime.datetime.now(datetime.timezone.utc),'%d-%m-%Y'))
...
class Page():
    ...
    def loadData(self):
        ...
        self.graph.renderer.node_renderer.data_source.on_change('selected',self.nodeSelectCallback)
        self.graph.renderer.node_renderer.data_source.trigger('selected',[],self.graph.renderer.node_renderer.data_source.selected)
        '''Data Table'''
